// Shared drama data — 4 dramas × 5 episodes.
...
const SERIES = [
  {
    ...
    episodes: [
      { ep: 1, title: '엄마의 부탁',       duration: 92 },
      { ep: 2, title: '바람잡이 친구',     duration: 88 },
      { ep: 3, title: '세 번째는 다르겠지', duration: 95 },
      { ep: 4, title: '카페에서 만난 남자', duration: 100 },
      { ep: 5, title: '우연은 두 번',       duration: 104 },
    ],
  },
  {
    id: 'midnight-store',
...
// Build feed for a given series (all released episodes + a coming-soon slot)
function getFeedFor(seriesId) {
  const s = getSeries(seriesId);
  if (!s) return [];
  const entries = s.episodes.map(e => ({
    id: `${s.id}-${e.ep}`,
    seriesId: s.id,
    ep: e.ep,
    epTitle: e.title,
  ...
  // Add coming-soon tail slot if there are more episodes to come
...
Object.assign(window, { SERIES, getSeries, getFeedFor, fmtTime });
